...
# for converting EPS to PDF
brew install ghostscript

sudo tlmgr update --self

sudo tlmgr install adjustbox caption graphics paralist makecell multirow chngcntr appendix csquotes biblatex tocloft nomencl algorithms algorithmicx listingsutf8 todonotes preview
```
...
The [my_thesis.tex](my_thesis.tex) and the produced [my_thesis.pdf](my_thesis.pdf) serves as an example of usage.
...
```bash
# Generate my_thesis.aux file
# (PDF file contains incorrect references yet)
pdflatex my_thesis.tex
# Generate bibliography
biber my_thesis
# Generate nomenclature (optional)
makeindex -s nomencl.ist -t my_thesis.nlg \
  -o my_thesis.nls my_thesis.nlo
